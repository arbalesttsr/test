...
class MbMenu extends CMenu
{
    ...
    protected function renderMenuRecursive($items)
    {
        foreach ($items as $item) {
            ...
            if (isset($item['url']))
                echo CHtml::link('<span>' . $item['label'] . '</span>', $item['url'], isset($item['htmlOptions']) ? $item['htmlOptions'] : array());
            else
                echo CHtml::link('<span>' . $item['label'] . '</span>', "javascript:void(0);", isset($item['htmlOptions']) ? $item['htmlOptions'] : array());
