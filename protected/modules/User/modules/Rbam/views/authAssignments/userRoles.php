            ...
            'buttons' => array(
                'revoke' => array(
                    ...
                    'click' => 'function() {
						...
						jDialog.html(jDialog.html().replace(/(.+? ").+?(::).+?(")/i,"$1"+"' . $user->username . '"+"$2"+jQuery("td:first",jRow).text()+"$3"));
                ...
                'update' => array(
                    'click' => 'function() {
						...
						jRow.addClass("selected");
						jQuery("#AuthAssignment_userName").val("' . $user->username . '");
...
$cs->registerScript(
    'rbamShowChildren', 'jQuery("td.children").on("click",function(){
		$.fn.rbam.showChildren(this);
	});'
);
$cs->registerScript(
    'rbamShowParents', 'jQuery("td.parents").on("click",function(){
