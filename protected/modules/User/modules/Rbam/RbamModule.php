...
class RbamModule extends CWebModule
{
    ...
    public function init()
    {
        ...
        if (preg_match('#Rbam#isu', $_SERVER["REQUEST_URI"])) {
            $this->_setBaseUrl();
        ...
        $this->setComponents(array(
            'analyser' => array(
                'class' => 'User.modules.Rbam.components.RbamAnalyser',
        ...
        if (!empty($this->initialise)) {
            $this->setComponents(array(
                'initialiser' => array(
                    'class' => 'User.modules.Rbam.components.RbamInitialiser',
    ...
    private function _publishModuleAssets()
    {
        ...
        if (!is_string($this->baseScriptUrl)) {
            ...
            $this->baseScriptUrl = ($this->development ?
                Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('application.modules.User.modules.Rbam.assets'), false, -1, true) :
                Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('application.modules.User.modules.Rbam.assets'))
            );
        }
//die(var_dump(Yii::getPathOfAlias('application.modules.User.modules.Rbam.assets')));
        $this->_cs->registerScriptFile($this->baseScriptUrl . '/js/jquery.rbam.js', CClientScript::POS_END);
//die(var_dump($this->_cs));
    ...
    public function getMenuItem($item = array())
    {
        ...
        $linkOptions = array_merge(
            ...
            (isset($item['htmlOptions']) ? $item['htmlOptions'] : array())
        );
        unset($item['htmlOptions']);
