...
Yii::import('User.modules.Rbam.extensions.alphapager.ApLinkPager');
Yii::import('User.modules.Rbam.extensions.alphapager.ApListPager');

class ApGridView extends CGridView
{
    /**
    ...
    public function renderAlphapager()
    {
        ...
        if ($this->ajaxUpdate !== false) {
            ...
            if ($class == 'ApLinkPager') {
                $cs->registerScript(__CLASS__ . '#' . $id, "jQuery('#" . $id . " ." . $this->alphaPagerCssClass . " a').bind('click',function(){ $.fn.yiiGridView.update('" . $id . "',{url: $(this).attr('href')});return false; });");
            } else if ($class == 'ApListPager') {
                $cs->registerScript(__CLASS__ . '#' . $id, "jQuery('#" . $id . " ." . $this->alphaPagerCssClass . " select').bind('change',function(){ $.fn.yiiGridView.update('" . $id . "',{url: $(this).val()});return false; });");
