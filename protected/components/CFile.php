...
class CFile extends CApplicationComponent
{
    ...
    private $_basename;
    /**
     * @var string _filename of the file (eg. 'myfile'
     * for '/var/www/htdocs/files/myfile.htm')
     */
    private $_filename;
    /**
     * @var string directory _filename of the filesystem object
    ...
    private $_isEmpty;
    /**
     * @var mixed filesystem object owner _filename (eg. 'idle') or
     * in ID (eg. '1000') see {@link getOwner} parameters
     */
    private $_owner;
    /**
     * @var mixed filesystem object group _filename (eg. 'apache') or
     ...
     * @return string Real file path
     */
    private static function realPath($suppliedPath, $dir_separator = DIRECTORY_SEPARATOR)
    {
...
//            Yii::trace('Path "'.$suppliedPath.'" resolved into "'.$realpath.'"', 'ext.file');
     ...
     * Returned value depends upon $get_filename parameter value.
     * If $_owner property is set, returned value is read from that property.
     *
     * @param boolean $get_filename Defaults to 'true', meaning that owner _filename
     * instead of ID should be returned.
     * @return mixed Owner _filename, or ID if $get_filename set to 'false'
     */
    public function getOwner($get_filename = true)
    {
        if (!isset($this->_owner))
            $this->_owner = $this->exists ? fileowner($this->_realpath) : null;

        if (is_int($this->_owner) && function_exists('posix_getpwuid') && $get_filename == true) {
            $this->_owner = posix_getpwuid($this->_owner);
            $this->_owner = $this->_owner['_filename'];
        }
     ...
     * For UNIX systems.
     *
     * @param mixed $owner New owner _filename or ID
     ...
     *
     * @param boolean $get_filename Defaults to 'true', meaning that group _filename
     * instead of ID should be returned.
     * @return mixed Group _filename, or ID if $get_filename set to 'false'
     */
    public function getGroup($get_filename = true)
    {
        if (!isset($this->_group))
            $this->_group = $this->exists ? filegroup($this->_realpath) : null;

        if (is_int($this->_group) && function_exists('posix_getgrgid') && $get_filename == true) {
            $this->_group = posix_getgrgid($this->_group);
            $this->_group = $this->_group['_filename'];
        }
     ...
     * For UNIX systems.
     *
     * @param mixed $group New group _filename or ID
     * @return mixed Current CFile object on success, 'false' on fail.
     */
     ...
     * Lazy wrapper for {@link re_filename}.
        ...
        if ($this->isFile) {
            ...
            if ($this->writeable && $extension !== false) {
                ...
                if (is_null($extension) || $extension == '') {
                    $newbasename = $this->file_filename;
                } // apply new extension
                else {
                    $extension = ltrim($extension, '.');

                    if (is_null($this->extension))
                        $newbasename = $this->file_filename . '.' . $extension;
                    else
                        $newbasename = str_replace($this->extension, $extension, $this->basename);
                }

                if ($this->re_filename($newbasename))
                    return $this;
            }
    ...
    /**
     * Returns the current file basename (file _filename plus extension) from
    ...
    public function setbasename($basename = false)
    {
        if ($this->isFile) {
            ...
            if ($this->writeable && $basename !== false && $this->re_filename($basename))
                return $this;
    ...
    /**
     * Re_filenames/moves the current filesystem object to specified destination.
     ...
     * object to be re_filenamed/moved to
     * @return mixed Updated current CFile object on success, 'false' on fail.
     */
    public function re_filename($fileDest)
    {
        $destRealPath = $this->resolveDestPath($fileDest);

        if ($this->writeable && @re_filename($this->_realpath, $destRealPath)) {
        ...
        $this->addLog('Unable to re_filename/move filesystem object into "' . $destRealPath . '"');
        return false;
    }

    /**
     * Resolves destination path for the current filesystem object.
     * This method enables short calls for {@link copy} & {@link re_filename} methods
     * (i.e. copy('mynewfile.htm') makes a copy of the current filesystem object
     * in the same directory, _filenamed 'mynewfile.htm')
     *
     * @param string $fileDest Destination filesystem object _filename
    ...
    private function pathInfo()
    {
        ...
        $pathinfo = pathinfo($this->_isUploaded ? $this->_uploadedInstance->get_filename() : $this->_realpath);

        $this->_dirname = $pathinfo['dirname'];
        $this->_basename = $pathinfo['basename'];

        // PHP version < 5.2 workaround
        if (!isset($pathinfo['file_filename'])) {
            $this->_filename = substr($pathinfo['basename'], 0, strrpos($pathinfo['basename'], '.'));
        } else {
            $this->_filename = $pathinfo['file_filename'];
        }
        if (key_exists('extension', $pathinfo))
            $this->_extension = $pathinfo['extension'];
        else
            $this->_extension = null;
        $this->name = $this->_filename . '.' . $this->_extension;
    }

    /**
     * Returns the current file _filename (without extension) from $_filename
     * property set by {@link pathInfo}
     * (eg. 'myfile' for '/var/www/htdocs/files/myfile.htm')
     *
     * @return string Current file _filename
     */
    public function getFile_filename()
    {
        return $this->_filename;
    }

    /**
     * Returns the current file directory _filename (without final slash) from
     * $_dirname property set by {@link pathInfo}
     * (eg. '/var/www/htdocs/files' for '/var/www/htdocs/files/myfile.htm')
     *
     * @return string Current file directory _filename
     */
    ...
    public static function set($filePath, $greedy = false)
    {
        if (trim($filePath) != '') {
            ...
            if (strpos($filePath, '\\') === false && strpos($filePath, '/') === false) {
                ...
                if ($uploaded) {
                    $filePath = $uploaded->getTemp_filename();
    ...
    /**
     * Sets the current file _filename.
     * Lazy wrapper for {@link re_filename}.
     * This method works only for files.
     *
     * @param string $file_filename New file _filename (eg. 'mynewfile')
     * @return mixed Current CFile object on success, 'false' on fail.
     */
    public function setFile_filename($file_filename = false)
    {
        if ($this->isFile) {
            if ($this->isUploaded) {
                $this->addLog(__METHOD__ . ' method is unavailable for uploaded files. Please copy/move uploaded file from temporary directory', 'warning');
            ...
            if ($this->writeable && $file_filename !== false &&
                $this->re_filename(str_replace($this->file_filename, $file_filename, $this->basename))
            )
                return $this;

            $this->addLog('Unable to set file _filename "' . $file_filename . '"');
            return false;
        }
    ...
    /**
     * Alias for {@link re_filename}
     */
    public function move($fileDest)
    {
        return $this->re_filename($fileDest);
    }
     ...
     * Alias for {@link send}
     */
    function download($fake_filename = false, $serverHandled = false)
    {
        return $this->send($fake_filename, $serverHandled);
    }
     ...
     * This method works only for files.
     *
     * @param string $fake_filename New file_filename (eg. 'myfileFaked_filename.htm')
     ...
     */
    public function send($fake_filename = false, $serverHandled = false)
    {
        if ($this->isFile) {
            if ($this->readable && !headers_sent()) {
                ...
                if ($fake_filename)
                    $file_filename = $fake_filename;
                else
                    $file_filename = $this->basename;
                ...
                header('Content-Disposition: attachment;file_filename="' . $file_filename . '"');
     ...
     * This method will use a local map between extension _filename and MIME type.
     * This method works only for files.
     *
     ...
     * success, 'false' on fail.
     */
    public function saveAs($fileDest)
    {

        $destRealPath = $fileDest;

        //die(var_dump($destRealPath));
        if ($this->isFile) {
    ...
    public function scandir($directory, $recursive = false, $filter = null)
    {
        $descendants = array();
